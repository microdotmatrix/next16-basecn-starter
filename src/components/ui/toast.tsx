...
function Toasts({ position = "bottom-right" }: { position: ToastPosition }) {
  ...
  const verticalSwipeDirection: SwipeDirection = isTop ? "up" : "down";
  const horizontalSwipeDirection: SwipeDirection = position.includes("left")
    ? "left"
    : "right";
  const swipeDirection: SwipeDirection[] = position.includes("center")
    ? [verticalSwipeDirection]
    : [horizontalSwipeDirection, verticalSwipeDirection];
            ...
            >
              <Toast.Content className="pointer-events-auto flex items-center justify-between gap-1.5 overflow-hidden px-3.5 py-3 text-sm transition-opacity duration-250 data-behind:pointer-events-none data-behind:opacity-0 data-expanded:opacity-100">
                <div className="flex items-center gap-2">
                  {Icon ? (
                    <div
                      className="[&>svg]:h-lh [&>svg]:w-8 [&_svg]:pointer-events-none [&_svg]:shrink-0"
...
function AnchoredToastItem({ toast }: { toast: ToastItem }) {
  const Icon = toast.type
    ? TOAST_ICONS[toast.type as keyof typeof TOAST_ICONS]
    : null;
  const tooltipStyle =
    (toast.data as { tooltipStyle?: boolean } | undefined)?.tooltipStyle ??
    false;
